...
public class Reset extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String wachtwoord1 = request.getParameter("txtWachtwoord");
            String wachtwoord2 = request.getParameter("txtBevestig");
            SessionFactory factory = HibernateFactory.getSessionFactory();
            boolean correct = true;
            if(wachtwoord1.equals("") || !wachtwoord1.equals(wachtwoord2)) {
                correct = false;
            }
            if(correct == true) {
                Session sessie = factory.openSession();
                Transaction tx = sessie.beginTransaction();
                TblPersoon persoon = (TblPersoon) sessie.load(TblPersoon.class, gebruiker);
                tx.commit();
                wachtwoord1 = EncryptionIni.encrypt(wachtwoord1);
                persoon.setWachtwoord(wachtwoord1);
